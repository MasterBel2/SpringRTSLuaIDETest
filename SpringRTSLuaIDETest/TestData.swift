...
let case2: (String) -> String = { startingText in
	var x = 0
	var newText = ""
	startingText.enumerateLines(invoking: { (line, _) in
		if x == 5 {
			guard let range = startingText.range(of: line) else {
				return
			}
...
function Console:init(channelName, sendMessageActions, noHistoryLoad, onResizeFunc, isBattleChat)
	...
	-- List of sent messages sent by the viewer. sentMessages[0] will be the message typed before retrieving
	-- sent messages if sentMessageIndex ~= 0.
	self.sentMessages = {}
	-- The index of the sent message currently being shown in the EditBox.
	self.sentMessageIndex = 0
	...
	self.ebInputText.OnKeyPress = {
		function(obj, key, mods, ...)
			...
			elseif key == Spring.GetKeyCode("up") then
				self:MoveForwardThroughSentMessages()
			elseif key == Spring.GetKeyCode("down") then
				self:MoveBackwardThroughSentMessages()
			end
		end
	}
...
function Console:MoveForwardThroughSentMessages()
	-- Don't move forward if not looking at a sent message.
	if historyIndex ~= 0 then
		self.FillSentMessage(self.sentMessageIndex - 1)
	end
end

-- Changes the current console text entry to the message sent before the one currently shown.
function Console:MoveBackwardThroughSentMessages()
	if historyIndex == 0 then
		-- Save current message before we move into the past.
		sentMessages[0] = self.ebInputText.text
	end
	self.FillSentMessage(self.sentMessageIndex + 1)
end

-- Changes the current console text entry to the specified sent message.
-- historyIndex must be >= 0
function Console:FillSentMessage(historyIndex)
	if historyIndex >= 0 then
		self.ebInputText:SetText(sentMessages[historyIndex])
...
function Console:SendMessage()
	if self.ebInputText.text ~= "" then
		...
		self.sentMessages[#self.sentMessages + 1] = message
